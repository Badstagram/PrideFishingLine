...
object YACLConfigPlatform {
    fun buildScreen(parent: Screen?): Screen {
        val builder =
            YetAnotherConfigLib.createBuilder().title(Text.literal("Pride Fishing Lines")).save(ConfigManager::saveConfig)
        ...
        val othersCategory = ConfigCategory.createBuilder().name(Text.literal("Others")).tooltip(Text.literal("Options for other players' fishing lines."))
            .options(listOf(
                Option.createBuilder<Boolean>()
                    ...
                    .available(false)
                    .description(OptionDescription.createBuilder().text(Text.literal("Hide other players' fishing lines"), Text.literal("Temporally Disabled due to Minecraft rendering changes")).build())
                    .binding(true,
