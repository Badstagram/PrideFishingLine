...
public class AgenderFishingLineRenderer implements FishingLineRenderer {
    ...
    public void render(float segmentStart, float segmentEnd, float f, float gg, float h, float i, float j, float k, MatrixStack.Entry matrices, VertexConsumer buffer) {
        ...
        if (segmentStart <= 12f / 14f && segmentStart > 10f / 14f)
            buffer.vertex(matrices.getPositionMatrix(), f, gg, h).color(188, 196, 198, 255).normal(matrices, i, j, k);
        ...
        if (segmentStart <= 8f / 14f && segmentStart > 6f / 14f)
            buffer.vertex(matrices.getPositionMatrix(), f, gg, h).color(182, 245, 131, 255).normal(matrices, i, j, k);
