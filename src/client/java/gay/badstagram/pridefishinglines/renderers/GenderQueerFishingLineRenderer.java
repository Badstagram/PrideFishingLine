...
public class GenderQueerFishingLineRenderer implements FishingLineRenderer {
    ...
    public void render(float segmentStart, float segmentEnd, float f, float gg, float h, float i, float j, float k, MatrixStack.Entry matrices, VertexConsumer buffer) {
        if (segmentStart <= 1f && segmentStart > 10f / 15f)
            buffer.vertex(matrices.getPositionMatrix(), f, gg, h).color(181, 126, 220, 255).normal(matrices, i, j, k);
        ...
        if (segmentStart <= 5f / 15f)
            buffer.vertex(matrices.getPositionMatrix(), f, gg, h).color(74, 129, 35, 255).normal(matrices, i, j, k);
    }
}
