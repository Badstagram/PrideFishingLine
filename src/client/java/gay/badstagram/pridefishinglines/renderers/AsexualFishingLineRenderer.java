...
public class AsexualFishingLineRenderer implements FishingLineRenderer {
    ...
    public void render(float segmentStart, float segmentEnd, float f, float gg, float h, float i, float j, float k, MatrixStack.Entry matrices, VertexConsumer buffer) {
        ...
        if (segmentStart <= 12f / 16f && segmentStart > 8f / 16f)
            buffer.vertex(matrices.getPositionMatrix(), f, gg, h).color(163, 163, 163, 255).normal(matrices, i, j, k);
        ...
        if (segmentStart <= 4f / 16f)
            buffer.vertex(matrices.getPositionMatrix(), f, gg, h).color(128, 0, 128, 255).normal(matrices, i, j, k);
    }
}
