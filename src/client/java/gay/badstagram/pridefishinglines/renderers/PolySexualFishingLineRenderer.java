...
public class PolySexualFishingLineRenderer implements FishingLineRenderer {
    ...
    public void render(float segmentStart, float segmentEnd, float f, float gg, float h, float i, float j, float k, MatrixStack.Entry matrices, VertexConsumer buffer) {
        if (segmentStart <= 1f && segmentStart > 10f / 15f)
            buffer.vertex(matrices.getPositionMatrix(), f, gg, h).color(246, 28, 158, 255).normal(matrices, i, j, k);
        if (segmentStart <= 10f / 15f && segmentStart > 5f / 15f)
            buffer.vertex(matrices.getPositionMatrix(), f, gg, h).color(7, 213, 105, 255).normal(matrices, i, j, k);
        if (segmentStart <= 5f / 15f)
            buffer.vertex(matrices.getPositionMatrix(), f, gg, h).color(28, 146, 246, 255).normal(matrices, i, j, k);
    }
}
